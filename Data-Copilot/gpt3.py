...
def create_sample_question(schema_data, data, regenerate_new_question):
    ...
    if len(goals_sample_question) > 0:
        ...
        prompt = f"You are an laymen person who wants to get general information from the data provided, " \
                 f"Generate me 50 questions based on data using the schema {schema_data}" \
                 f"please generate the questions to meet the objective of {goals_sample_question}" \
                 f"Put each question in <question_start> your generated question <question_end>."
    else:
        # prompt = f"You are an data analyst, " \
        ...
        prompt = f"You are an laymen person who wants to get general information from the data provided, " \
                 f"Generate me 50 questions based on data using the schema {schema_data}" \
                 "The questions to ask should be all based on data like: " \
                 "If the data is about employee, you can ask: " \
                 "How many employees joined the company in 2020? " \
                 "Show the list of employees who are working in a particular department" \
                 "How many employees are working from a particular region? " \
                 "Trend of employees joining the company over the years? " \
                 "Distribution of employees with a particual designation" \
                 "Show the list of employees from a particular country" \
                 f"Put each question in <question_start> your generated question <question_end>."
...
@st.cache_data(show_spinner=False)
def generate_sql_gpt(_data_schema, new_question, _sample_data, _schema_data2, _sample_data2):
    if _schema_data2 is None and _sample_data2 is None:
        prompt = f"""
        ...
        Example Context: 
        You are a data analyst and given with two data schemas and two data samples. The reson two schemas and samples are are given could be that the user is looking ofr some data which could require join queries between the 
        given two schemas.
        Given the data with schema:
        dict_items([('key', string[python]), ('author', string[python]), ('date', dtype('<M8[ns]')), ('stars', Int64Dtype()), ('title', string[python]), ('helpful_yes', Int64Dtype()), ('helpful_no', Int64Dtype()), ('text', string[python])])
        ...
        SELECT COUNT(*) 
        FROM DATA 
        JOIN DATA2 ON DATA.employee_id = DATA2.employee_id
        ...
        FROM DATA 
        JOIN DATA2 ON DATA.employee_id = DATA2.employee_id
        WHERE DATA2.laptop_model = 'macbook air' AND
        DATA.year_of_joining = 2018;
        <sql_end>
        ...
        You are a data analyst and given with two data schemas and two data samples. The reson two schemas and samples are are given could be that the user is looking ofr some data which could require join queries between the 
        given two schemas. 
        Given the data with schemas are: 
        {_data_schema} and {_schema_data2}
        Given sample data are:
        {_sample_data} and {_sample_data2}
